...
function energyLevelBottom(level: number) {
  return level === 1 ? 5 : 15 + (level - 2) * 14;
}
...
export default function Home() {
  ...
  return (
    <main>
      ...
      <section id="hydrogen" className="dark-section page-section"><div className="section-wrap"><SectionHeading number="03" kicker="LAB 02 / QUANTIZED ENERGY" title="Hydrogen leaves fingerprints." >An electron can occupy only specific energy levels. A jump between levels creates a photon with a precise wavelength.</SectionHeading><CompletionButton id="hydrogen" completed={completed.has("hydrogen")} onComplete={completeSection} /><div className="hydrogen-grid"><div className="energy-ladder"><div className="ladder-label">ENERGY / J</div>{[1, 2, 3, 4, 5, 6].map((level) => <div className="energy-level" key={level} style={{ bottom: `${energyLevelBottom(level)}%` }}><span>n = {level}</span><i /><small>{formatScientific(hydrogenEnergy(level))}</small></div>)}<div className="transition-line" style={{ bottom: `${Math.min(energyLevelBottom(initialLevel), energyLevelBottom(finalLevel))}%`, height: `${Math.abs(energyLevelBottom(initialLevel) - energyLevelBottom(finalLevel))}%` }} /><div className="nucleus-dot" /></div><div className="hydrogen-controls"><p className="eyebrow">SET A TRANSITION</p><div className="select-row"><label>Electron starts at <select value={initialLevel} onChange={(event) => setInitialLevel(Number(event.target.value))}>{[1, 2, 3, 4, 5, 6].map((level) => <option key={level} value={level}>n = {level}</option>)}</select></label><label>Electron ends at <select value={finalLevel} onChange={(event) => setFinalLevel(Number(event.target.value))}>{[1, 2, 3, 4, 5, 6].map((level) => <option key={level} value={level}>n = {level}</option>)}</select></label></div><div className={`transition-card ${initialLevel > finalLevel ? "emission" : "absorption"}`}><span>{initialLevel > finalLevel ? "EMISSION" : initialLevel < finalLevel ? "ABSORPTION" : "NO JUMP"}</span><strong>{initialLevel === finalLevel ? "Choose two levels" : `${Math.abs(initialLevel - finalLevel)} level ${initialLevel > finalLevel ? "drop" : "rise"}`}</strong><p>{initialLevel === finalLevel ? "An electron must change levels to exchange energy." : `${formatScientific(transition.photonEnergy)} J photon · ${transition.wavelengthNm.toFixed(1)} nm`}</p></div><div className="hydrogen-note"><b>Why lines?</b> The allowed energies are discrete, so only certain photon energies—and therefore only certain colors—can appear.</div></div></div></div></section>
